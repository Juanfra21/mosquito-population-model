...
def load_population_data(population_path):
    ...
    # Create date column
    population_df['Date'] = population_df.apply(lambda row: datetime.datetime(row['YEAR'], 1, 1) + datetime.timedelta(days=row['DOY'] - 1), axis=1)
...
def interpolate_population(population_df):
    ...
    date_range = pd.date_range(start=population_df.index.min(), end=population_df.index.max(), freq='D')
...
def create_future_population(population_df):
    ...
    population_df['population_2d'] = population_df['population'].rolling(window=2, min_periods=1).mean().shift(-1)

    # Calculate the average population of the current day and the next three days
    population_df['population_3d'] = population_df['population'].rolling(window=3, min_periods=1).mean().shift(-2)
