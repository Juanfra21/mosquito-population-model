...
def test_train_split_lstm(data, target, train_size, test_from_start, batch_size):
    ...
    return train_loader, val_loader, test_loader, X_train.values, X_val.values, X_test.values,y_train.values, y_val.values, y_test.values


def train_model(train_loader, val_loader, input_size, criterion):
    model = LSTMModel(input_size, CONFIG['model']['hidden_size'], 
